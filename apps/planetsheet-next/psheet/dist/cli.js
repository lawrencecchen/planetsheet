...
var require_tiny_invariant_cjs = __commonJS({
  "node_modules/tiny-invariant/dist/tiny-invariant.cjs.js"(exports, module2) {
    "use strict";
...
function getConnectionString() {
  ...
  throw new Error(`No connection string found. Please set one of the following environment variables: ${searchEnvVars.join(", ")}`);
}
function getConnection(connectionString2) {
  if (globalThis.__planetsheetConnection) {
    return globalThis.__planetsheetConnection;
  }
  if (connectionString2 == null ? void 0 : connectionString2.includes("postgres")) {
    return (0, import_knex.default)({
      client: "pg",
      connection: connectionString2,
      searchPath: ["public", "information_schema"],
      pool: { min: 0, max: 7 }
    });
  }
  if (connectionString2 == null ? void 0 : connectionString2.includes("mysql")) {
    return (0, import_knex.default)({
      client: "mysql",
      connection: connectionString2
    });
...
function connectionStringSSLHeuristic(connectionString2) {
  const url = new URL(connectionString2);
  if (connectionString2 == null ? void 0 : connectionString2.includes("postgres")) {
...
var connectionStringHeuristics = [
  (connectionString2) => connectionString2,
  (connectionString2) => connectionStringSSLHeuristic(connectionString2)
];
async function getConnectionWithHeuristics(connectionString2, iTry = 0, iMax = connectionStringHeuristics.length) {
  if (iTry === iMax) {
    throw new Error("Could not connect to database.");
  }
  const _connectionString = connectionStringHeuristics[iTry](connectionString2);
  ...
  try {
    ...
    _connection.destroy();
    return getConnectionWithHeuristics(connectionString2, iTry + 1, iMax);
  }
}
var connectionString = getConnectionString();
(0, import_tiny_invariant.default)(connectionString, "No connection string found.");
var db = getConnection(connectionString);
...
var package_default = {
  name: "planetsheet",
  version: "0.1.21",
  description: "The most user friendly database tool.",
  keywords: [
    "postgres",
    "mysql"
  ],
  scripts: {
    dev: "run-p dev:*",
    "dev:next": "next dev",
    "dev:unocss": "npm run generate:unocss -- --watch",
    build: "run-s build:*",
    "build:unocss": "npm run generate:unocss",
    "build:next": "next build",
    "build:prepare_standalone": "tsx ./utils/prepare_standalone.ts",
    "cli:dev": "tsx ./cli/cli.ts",
    start: "next start -p 54337",
    lint: "next lint",
    "generate:unocss": 'unocss "./src/**/*.{ts,tsx,jsx,js}" --out-file ./styles/uno.css',
    typecheck: "tsc -b",
    prettier: "prettier --write app",
    format: "prettier --write .",
    prepublishOnly: "npm run lint && npm run build",
    postpublish: "cd psheet && npm publish"
  },
  dependencies: {
    "@faker-js/faker": "^7.3.0",
    "@glideapps/glide-data-grid": "^4.2.1",
    "@iconify/json": "^2.1.77",
...
// cli/lib/startPlanetsheetStudio.ts
var import_node_path = __toESM(require("path"));
var import_node_fs = __toESM(require("fs"));
var import_dotenv2 = __toESM(require_main());
import_dotenv2.default.config();
function startPlanetsheetStudio() {
  const cliDir = import_node_path.default.dirname(import_node_fs.default.realpathSync(__filename));
  function assertConnectionString() {
    ...
    console.error(`No connection string found. Please set one of the following environment variables: ${searchEnvVars.join(", ")}`);
    process.exit(1);
  }
  assertConnectionString();
  function startServer() {
    const serverDir = import_node_path.default.join(cliDir, "./server.js");
    if (!import_node_fs.default.existsSync(serverDir)) {
      console.error(`Server file not found: ${serverDir}`);
      process.exit(1);
    }
    const port = process.env.PORT || String(58337);
    const serverSource = import_node_fs.default.readFileSync(serverDir, "utf8").replace("3000", port);
    try {
      const runServer = new Function("__dirname", "require", `(() => {
${serverSource}
})()`);
  ...
  const connectionString2 = getConnectionString();
  if (!connectionString2) {
    console.error("No connection string found.");
    process.exit(1);
  }
  try {
    getConnectionWithHeuristics(connectionString2).then((connection) => {
...
// cli/cli.ts
yargs_default(hideBin(process.argv)).command("$0", "Launch Planetsheet studio", () => {
}, () => {
  console.log(`Planetsheet v${package_default.version}`);
  startPlanetsheetStudio();
}).command("psql", "Launch `psql` using your .env DATABASE_URL.", () => {
}, (argv) => {
  const connectionString2 = getConnectionString();
  console.log(connectionString2);
  (0, import_node_child_process.exec)(`psql ${connectionString2}`, {});
